...
//   node video/engine/scripts/audio.mjs --video <slug> --audition      # only the voice audition (.audition/*.mp3)
//   node video/engine/scripts/audio.mjs --video <slug> --only s08-02 --force
//
// Every step runs with VIDEO=<slug> in its environment, so they all act on the same video.
//
...
function step(label, cmd, args, env = {}) {
  console.log(`\n== ${label}`);
  const res = spawnSync(cmd, args, { cwd: REPO_ROOT, stdio: 'inherit', env: { ...process.env, VIDEO, ...env }, windowsHide: true });
  if (res.error) {
