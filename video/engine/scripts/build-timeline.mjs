#!/usr/bin/env node
// Builds video/<slug>/src/timeline.json (the only input the Remotion composition reads) and
...
//
//   node video/engine/scripts/build-timeline.mjs --video <slug>            # audio mode (needs audio.mjs first)
//   node video/engine/scripts/build-timeline.mjs --video <slug> --estimate # no audio: 2.5 words/s estimate
//
// The profile (length window, style counts, chapter ceiling) and the transcript's
// disclaimer come from video/<slug>/video.json; the transcript title is storyboard.title.
//
...
async function loadAudio(segments, voice, opts, errors, warnings) {
  ...
  for (const seg of segments) {
    ...
    if (!existsSync(jsonPath)) {
      errors.push(`${seg.id}: no TTS data at ${jsonPath} — run: ${AUDIO_CMD}`);
      continue;
    }
    ...
    if (tts.key !== want) {
      ...
      errors.push(`${seg.id}: TTS is stale (${why.join('; ') || 'key mismatch'}) — run: ${AUDIO_CMD}`);
      continue;
    }
...
export async function buildTimeline(options = {}) {
  ...
  if (totalSec < profile.minTotalSec || totalSec > profile.maxTotalSec) {
    warnings.push(
