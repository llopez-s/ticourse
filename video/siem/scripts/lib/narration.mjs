 ...
 */
export function analyzeNarration({ storyboard, narration, lexicon }) {
  const errors = [];
  const warnings = [];
  ...
  if (thinkScenes.length !== 2) warnings.push(`${thinkScenes.length} think prompts (style guide: exactly 2, in s08-triage and s10-contain)`);
  if (examCount && (examCount < 8 || examCount > 11)) warnings.push(`${examCount} exam cards (style guide: about 9–10)`);
