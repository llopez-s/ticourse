 ...
 * @param {{sceneIds?: string[]}} [opts] allowed scene ids (defaults to the SceneId union)
 * @returns {string[]} errors (empty when valid)
 */
export function validateTimeline(t, opts = {}) {
  const sceneIds = new Set(opts.sceneIds ?? SCENE_IDS);
  const errors = [];
  ...
  t.scenes.forEach((s, k) => {
    ...
    if (!sceneIds.has(s.id)) errors.push(`${where}.id: unknown scene ${JSON.stringify(s.id)}`);
    if (![1, 2, 3, 4, 5].includes(s.chapter)) errors.push(`${where}.chapter: must be 1..5`);
