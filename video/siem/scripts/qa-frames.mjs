...
//
//   node video/siem/scripts/qa-frames.mjs                    # all scenes -> out/qa/all/
//   node video/siem/scripts/qa-frames.mjs --scene s05-correlate
//   node video/siem/scripts/qa-frames.mjs --extra 1200,1350
//
...
import { assertCliFlags, runRemotion } from './lib/remotion.mjs';
...
async function main() {
  ...
  const common = ['--image-format=jpeg', '--jpeg-quality=85', `--scale=${values.scale}`, `--public-dir=${PATHS.publicDir}`];
  ...
  if (supportsFrameLists()) {
    const res = runRemotion(['render', PATHS.entry, COMPOSITION, outDir, '--sequence', `--frames=${frames.join(',')}`, ...common]);
    ...
    // Fallback: bundle once, then one still per frame from the same bundle.
    const bundleDir = mkdtempSync(path.join(os.tmpdir(), 'siem-qa-bundle-'));
    try {
      const b = runRemotion(['bundle', PATHS.entry, `--out-dir=${bundleDir}`, `--public-dir=${PATHS.publicDir}`]);
      if (b.status !== 0) throw new Error(`remotion bundle failed (exit ${b.status})`);
      for (const frame of frames) {
        const file = path.join(outDir, `element-${frame}.jpeg`);
        const res = runRemotion(['still', bundleDir, COMPOSITION, file, `--frame=${frame}`, ...common]);
