...
//
//   node video/siem/scripts/render.mjs            # public/videos/siem-blue-team.mp4 + -poster.png
//   node video/siem/scripts/render.mjs --draft    # half scale, crf 30 -> video/siem/out/draft.mp4
//
// Refuses to run unless src/timeline.json is in audio mode, its sourceHash
// matches the current sources and every voice clip exists.
import { mkdirSync, statSync } from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import { parseArgs } from 'node:util';
import { checkTimelineFresh } from './lib/freshness.mjs';
import { COMPOSITION, PATHS, POSTER_STILL, isMainModule } from './lib/paths.mjs';
import { assertCliFlags, probeMedia, runPool, runRemotion, runRemotionAsync } from './lib/remotion.mjs';

const SIZE = { targetMin: 15, targetMax: 25, warn: 30, fail: 45 }; // MB (10^6 bytes)
...
async function main() {
  ...
  const flags = ['public-dir', 'codec', 'crf', 'x264-preset', 'pixel-format', 'audio-codec', 'audio-bitrate', 'image-format', 'jpeg-quality', 'concurrency', 'enforce-audio-track', 'scale'];
  ...
  if (!draft) run('poster', ['still', PATHS.entry, POSTER_STILL, PATHS.poster, `--public-dir=${PATHS.publicDir}`, '--image-format=png']);
