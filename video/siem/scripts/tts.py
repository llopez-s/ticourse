#!/usr/bin/env python3
"""Synthesise the SIEM narration with edge-tts (one MP3 + word timings per segment).

    python -X utf8 video/siem/scripts/tts.py [--narration PATH] [--lexicon PATH]
...
SIEM = SCRIPTS.parent
DEFAULTS = {
    "narration": SIEM / "narration.json",
    "lexicon": SIEM / "lexicon.json",
    "storyboard": SIEM / "storyboard.json",
    "voice_dir": SIEM / "public" / "voice",
    "tts_dir": SIEM / "tts",
    "input": SIEM / "out" / "tts-input.json",
    "audition_dir": SIEM / ".audition",
}
